...
@section('table')
  ...
  @section('title')
  <h2 class="text-2xl font-semibold text-gray-900">Entrées/Sorties</h2>
  @stop
  ...
  <div class="my-5">
          ...
          <div class="relative bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:max-w-sm sm:w-full sm:p-6">
            ...
            <form action="{{ route('flows.store') }}" method="post" enctype="multipart/form-data" class="form m-6">
              ...
              <div class="border border-gray-300 rounded-md px-3 py-2 shadow-sm focus-within:ring-1 focus-within:ring-indigo-600 focus-within:border-indigo-600">
                  ...
                  <input type="text" name="title" id="title" class="focus:ring-indigo-500 focus:border-indigo-500 block w-full pl-7 pr-12 sm:text-sm border-gray-300 rounded-md" 
              ...
              <div class="border border-gray-300 rounded-md px-3 py-2 shadow-sm focus-within:ring-1 focus-within:ring-indigo-600 focus-within:border-indigo-600">
                ...
                <div class="mt-1 relative rounded-md shadow-sm">
                  <input type="text" id="categories" name="categories" 
                    class="focus:ring-indigo-500 focus:border-indigo-500 block w-full pl-7 pr-12 sm:text-sm border-gray-300 rounded-md" 
                    placeholder="Votre catégorie" aria-describedby="catégories">
                </div>
              </div>
              <div class="border border-gray-300 rounded-md px-3 py-2 shadow-sm focus-within:ring-1 focus-within:ring-indigo-600 focus-within:border-indigo-600">
                  <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                    ...
                    <td class="px-6 py-4">
                      <button type="button" 
                        class="all_category inline-flex items-center px-2.5 py-1.5 border border-transparent text-xs font-medium rounded shadow-sm 
              ...
              <div class="border border-gray-300 rounded-md px-3 py-2 shadow-sm focus-within:ring-1 focus-within:ring-indigo-600 focus-within:border-indigo-600">
                <select name="flow" 
                  class="form-select appearance-none block w-full text-base font-normal text-gray-700 
                    bg-white bg-clip-padding bg-no-repeat border border-solid border-gray-300 rounded transition ease-in-out m-0 focus:text-gray-700 
              ...
              <button type="submit" 
                class="submit_btn inline-flex justify-center w-full rounded-full mt-6 shadow-sm text-white bg-indigo-600 
            ...
            <div class="mt-5 sm:mt-6">
              <button type="button" class="modal_bg inline-flex justify-center w-full rounded-md border border-transparent shadow-sm px-4 py-2 
                bg-indigo-600 text-base font-medium text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 
                focus:ring-indigo-500 sm:text-sm">Fermer le modal</button>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="border border-gray-300 rounded-md px-3 py-2 shadow-sm focus-within:ring-1 focus-within:ring-indigo-600 focus-within:border-indigo-600">
      <p class="text-2xl font-semibold text-gray-900">Cliquez ici pour créer une entrée ou une sortie</p>
          ...
          @foreach ($flows as $flow)
            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                ...
                <td class="px-6 py-4 text-right">
                    <a href="{{ route('flows.edit', $flow) }}" class="edit font-medium text-blue-600 dark:text-blue-500 hover:underline">Modifier</a>
                </td>
                <td class="px-6 py-4 text-right">
                  <a href="#" class="font-medium text-blue-600 dark:text-blue-500 hover:underline" onclick="event.preventDefault; document.getElementById('destroy-flow-form').submit();">Supprimer
                    <form action="{{ route('flows.destroy', $flow->id) }}" method="post" id="destroy-flow-form">
  ...
  <nav class="bg-white px-4 py-3 flex items-center justify-between border-t border-gray-200" aria-label="Pagination">
    <div class="flex-1 flex justify-between">
      <a href="#" class="relative inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:text-gray-500"> Précédent </a>
      <a href="#" class="ml-3 relative inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:text-gray-500"> Suivant </a>
    </div>
  </nav>
  ...
  <script type="text/javascript">
    ...
    const inflow_update = document.getElementById('inflow_update');
    const outflow_update = document.getElementById('outflow_update');
    const flow_ = document.getElementById('flow');

    const edit = document.querySelectorAll('.edit');
    const title = document.getElementById('title_update');
    const value = document.getElementById('value_update');
    const cat = document.getElementById('categories_update');
